...
It does **not** edit `~/.codex/config.toml` unless you pass `--patch-config` / `-PatchConfig`.

## Install and patch config

macOS/Linux:

```bash
./install.sh --patch-config
```

Windows PowerShell:

```powershell
./install.ps1 -PatchConfig
```
...
```bash
./install.sh --plugin --patch-config
```

Windows PowerShell:

```powershell
./install.ps1 -Plugin -PatchConfig
```

The plugin packaging makes the skills available through Codex's plugin system. The config-patched hook gives the always-on prompt gate.
...
- `so_tester`: test and verification agent.
